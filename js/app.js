...
var audioChunks = [];
var isNodeApp = false;
var videoTimeout = 500;
var universalPageNum ;
var penSizeTurn ;
...
function reply_click(clicked)
{
    ...
    var eles = document.querySelectorAll('#book iframe');
			var eles1 = eles.length;
			console.log("length "+eles1);
			for(var i=0; i<eles.length; i++) {
				document.querySelectorAll('#book iframe')[i].contentWindow.postMessage(penSizeTurn, "*");
			}
...
var readerApp = {
	...
	'setPageNumBack': function() {
		...
		for(var i=0; i<currentPages.length; i++) {
			//console.log("Current Page :"+currentPages[i]);
			getNotes(currentPages[i]);

			var page = currentPages[i];

			universalPageNum = page;

			console.log("universalPageNum :"+page);

			if (page == 4) 
			{
				//$("#reportsBtn").attr("hidden",false);
				//$("#reportsBtn").css("display","block");					
				/*var element = $('<div />', {});
				element.html('<div style="z-index2;position:absolute"> <button style="width:10%;height:10px;background:#000000"></button></div>');
				$("#t1").attr("hidden",false);
		    	$("#t2").attr("hidden",false);
		    	$("#t3").attr("hidden",false);*/
			...
			switch(currentPage)
		{
			...
			case 1:
			element.html('<div position:absolute;z-index:2> <button style="width:10%;height:10%;background:#000000"></button></div>');
			break;
		}

			console.log("page App.js :"+currentPages[i]);
*/
		}
				...
				for(var i=0; i<bookmarksList.length; i++) {
					bookingHTML += '<li>\
                            <a href="#" onClick="readerApp.turnPage(' + bookmarksList[i] + ')">\
                                <div>\
                                    <i class="fa fa-chevron-right fa-fw"></i> Page ' + (bookmarksList[i]-appdb.config.prePages.length) + '\
                                </div>\
                            </a>\
                        </li>';
			...
			for(var i=0; i<dataObj.length; i++) {
				$highlightContainer.append('<li>\
					<a href="#" onClick="readerApp.turnPage(' + dataObj[i] + ')">\
						<div>\
							<i class="fa fa-chevron-right fa-fw"></i> <b>Page ' + (parseInt(dataObj[i]) - appdb.config.prePages.length) + '</b> \
						</div>\
					</a>\
				</li>');
			}
	...
	'prepareNoteAdd': function(_self) {
		...
		for(var i=0; i<pages.length; i++) {
			selectHTML += '<option value="' + pages[i] + '">Page ' + (parseInt(pages[i]) - appdb.config.prePages.length) + '</option>'
		}
				...
				for(j=0; j<myNote.length; j++) {
					...
					$('#notes-containers').append('<li>\
                            <a href="#" onClick="readerApp.turnPage(' + i + ')">\
                                <div>\
                                    <i class="fa fa-chevron-right fa-fw"></i> <b>Page ' + (i-appdb.config.prePages.length) + '</b> ' + textOnly + '\
                                </div>\
                            </a>\
                        </li>');
				}
	...
	draw: {
		...
		undo: function() {
			var currentPageId = parseInt($("#tools-pageselection").val()) + appdb.config.prePages.length;
	...
	loadThumbnail: function() {
		...
		for(var i=1; i<=eBookConfig.total; i++) {
			thumbailHTML += '<div class="item page-thumbnail" id="thumbnail-page-' + i + '" data-page="' + i + '">\
					<img src="resources/book/file-page' + i + '.jpg" />\
					<div class="thumbnail-page">' + i + '</div>\
				</div>'
		}
		$("#book-thumbnail").html('<div class="owl-carousel owl-theme">' + thumbailHTML + '</div>');
		...
		$(".thumbnail").off().on("click", ".page-thumbnail", function(e) {
			e.preventDefault();
			e.stopPropagation();
...
$(document).ready(function(e) {
	...
	/*$("#size-selection").on('change', function(e) {
		var pensize = $(this).val();
		$(this).css('width', pensize);
		var message = "select-width-" + pensize;
		var eles = document.querySelectorAll('#book iframe');
		for(var i=0; i<eles.length; i++) {
			eles[i].contentWindow.postMessage(message, "*");
		}
...
$(window).on("message", function(e) {
	...
	if(data == 'refresh-highlights') {
		...
		$("#tools-pageselection").val(parseInt(splitData[1]) - appdb.config.prePages.length);
...
function checkPageNumber1(realPageNum)
{

	var newID = "unhideimage"+realPageNum;
	var imgTag = document.getElementById(newID);
	//console.log(imgTag);
	var counter1 = 1;
	
	//imgTag.setAttribute('id', newID);
	/*if (universalPageNum == realPageNum) 
	{*/
	
	//console.log("UniversalPageNum pages :"+universalPageNum);
	//console.log("RealPageNum pages :"+newID);
	
		if (counter1 == 1 && imgTag.style.visibility == 'hidden' ) 
		{
		...
		else if (imgTag.style.visibility == 'visible' ) 
		{
...
function checkPageNumber2(realPageNum2)
{
	
	var newID = "unhideimage"+realPageNum2;
	var imgTag2 = document.getElementById(newID);
	//console.log(imgTag);
	var counter2 = 1;
...
/*	if (universalPageNum == realPageNum) 
	...
	*/
		...
		if (counter2 == 1 && imgTag2.style.visibility == 'hidden') 
		{
			imgTag2.style.visibility = 'visible';
			counter2++;
		}
		else if (imgTag2.style.visibility == 'visible') 
		{
			imgTag2.style.visibility = 'hidden';
			counter2 = 1;
		}
...
function checkPageNumber3(realPageNum3)
{
	
	var newID = "unhideimage"+realPageNum3;
	var imgTag3 = document.getElementById(newID);
	//console.log(imgTag);
	var counter3 = 1;
	...
	{*/
		...
		if (counter3 == 1 && imgTag3.style.visibility == 'hidden') 
		{
			imgTag3.style.visibility = 'visible';
			counter3++;
		}
		else if (imgTag3.style.visibility == 'visible') 
		{
			imgTag3.style.visibility = 'hidden';
			counter3 = 1;
		}
	/*}*/
...
/*function openFancyModalVideo(url, dimension) {
	...
	$.fancybox.open({
        ...
        opts: {
            ...
            width: dimension.split('x')[0], // Set the width (e.g., '1280x720' -> '1280')
            height: dimension.split('x')[1], // Set the height (e.g., '1280x720' -> '720')
        }
    });
}
*/
